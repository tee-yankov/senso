                        ...
                        let chunks = Layout::default()
                            ...
                            .constraints(
                                [
                                    ...
                                    Constraint::Percentage(100 - app.block_size.input),
                                ]
                                .as_ref(),
                            )
                            .split(chunks[0]);
                        app.draw_user_input(frame, chunks[0], &events.tx);
                        app.draw_kernel_info(frame, chunks[1], &kernel.info.current_info);
                    }
            ...
            Event::Input(input) => {
                ...
                if app.input_mode.is_none() {
                    ...
                    match input {
                        ...
                        Key::Alt('s') => {
                            let block_size = app.block_size();
                            *block_size = (*block_size).checked_sub(5).unwrap_or_default()
                        }
                        ...
                        Key::Char('<') | Key::Alt(' ') => {
                            ...
                            kernel.modules.scroll_mod_info(ScrollDirection::Up, false)
                        }
                        /* Scroll module information down. */
                        Key::Char('>') | Key::Char(' ') => {
                            app.selected_block = Block::ModuleInfo;
                            kernel.modules.scroll_mod_info(ScrollDirection::Down, false)
                        }
                        ...
                        Key::Ctrl('l') | Key::Ctrl('u') | Key::Alt('c') | Key::Alt('C') => {
                            kernel
                                .modules
                                .set_current_command(ModuleCommand::Clear, String::new());
                        }
                        ...
                        | Key::Delete => {
                            kernel
                                .modules
                                .set_current_command(ModuleCommand::Blacklist, String::new());
                        }
                        /* Reload kernel module. */
                        Key::Ctrl('r') | Key::Ctrl('R') | Key::Alt('r') | Key::Alt('R') => {
                            kernel
                                .modules
                                .set_current_command(ModuleCommand::Reload, String::new());
                        }
                        /* Execute the current command. */
                        Key::Char('y') | Key::Char('Y') => {
                            if kernel.modules.execute_command() {
                                events.tx.send(Event::Input(Key::Char('r'))).unwrap();
                            }
                        }
                        ...
                        Key::Char('c') | Key::Char('C') => {
                            app.set_clipboard_contents(match app.selected_block {
                                ...
                                Block::ModuleInfo => &kernel.modules.current_info.raw_text,
                                Block::Activities => kernel.logs.selected_output.trim(),
                                _ => "",
                            });
                        }
                            ...
                            if input == Key::Char('\n') && app.show_options {
                                if let Ok(command) = ModuleCommand::try_from(
                                    ...
                                    if command == ModuleCommand::Load {
                                        events.tx.send(Event::Input(Key::Char('+'))).unwrap();
                                    } else {
                                        kernel.modules.set_current_command(command, String::new());
                                    }
                                } else {
                                    match app.options.selected().map(|(v, _)| v.as_ref()) {
                                        Some("dependent") => {
                                            app.show_dependent_modules(&mut kernel.modules);
                                        }
                                        Some("copy") => {
                                            app.set_clipboard_contents(&kernel.modules.current_name)
                                        }
                                        _ => {}
                                    }
                                }
                    ...
                    match input {
                        ...
                        Key::Down => {
                            app.input_mode = match app.input_mode.next() {
                                Some(v) => v,
                                None => InputMode::first().and_then(|v| v.next()).unwrap(),
                            };
                            app.input_query = String::new();
                        }
                        ...
                        | Key::Left => {
                            ...
                            app.selected_block = match input {
                                ...
                                Key::Char('\n') => match app.input_mode {
                                    InputMode::Load if !app.input_query.is_empty() => {
                                        Block::ModuleInfo
                                    }
                                    _ => Block::ModuleTable,
                                },
                            ...
                            if app.input_mode == InputMode::Search && kernel.modules.index == 0 {
                                kernel.modules.scroll_list(ScrollDirection::Top);
                            /* Load kernel module. */
                                ...
                                && !app.input_query.is_empty()
                            {
                                kernel
                                    .modules
                                    .set_current_command(ModuleCommand::Load, app.input_query);
                                app.input_query = String::new();
                            }
